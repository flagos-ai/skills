...
|  | 插件环境搭建 | [`vllm-plugin-fl-setup-flagos`](skills/vllm-plugin-fl-setup-flagos/) | 在多种硬件后端（NVIDIA、昇腾、MetaX、天数智芯、摩尔线程等）上安装和配置 vLLM-Plugin-FL。自动化完整搭建流程：检测硬件 → 安装 vLLM-Plugin-FL → 安装 FlagGems → （可选）安装 FlagCX → 后端特定配置 → 推理验证。触发词如 "setup vllm-plugin-fl"、"install vllm-plugin-fl"、"configure FL plugin"、"set up FlagGems"、"set up FlagCX"。 |
...
|  | MCP 服务配置 | [`kernelgen-flagos`](skills/kernelgen-flagos/kernelgen-mcp-setup.md) | 自动检测并配置 `kernelgen-mcp` MCP 服务。检查项目本地配置文件中是否已有配置，如未配置则引导用户获取 Token 并自动写入配置。在任何生成/优化/特化子技能执行前运行。 |
