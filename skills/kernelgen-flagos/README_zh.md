...
KernelGen `kernelgen-flagos` 技能（Skill）是北京智源人工智能研究院（BAAI）推出的统一 AI 编程技能。通过调用 `kernelgen-mcp` KernelGen 算子开发 MCP 工具集服务，该技能兼容国内外多种芯片平台，可生成高精度 GPU 算子。它能够自动检测目标代码仓库的类型，并分发至对应的专属工作流执行。

## 快速入门

本节介绍如何快速启动一个任务，例如为特定平台创建、优化、专化算子，或提交反馈。您可以使用提示词或命令行来执行这些任务。本节仅介绍如何使用提示词。如需了解命令行或手动配置的使用方式，请参见 [KernelGen 技能用户指南](https://docs.flagos.io/projects/kernelgen/en/latest/skills_user_guide/skills-user-guide.html#)。
...
2. 在您的 AI 智能体客户端中，发送提示词以连接 KernelGen 算子开发 MCP Toolkit，例如：
...
3. 验证 KernelGen 算子开发 MCP Toolkit 连接，发送提示词："请验证 kernelgen MCP 连接是否成功。"

4. 发送提示词以安装 `kernelgen-flagos` 统一技能（包含所有子技能），例如：

   * **Copilot**："在 **Copilot 中**从 <https://github.com/flagos-ai/skills/tree/main/skills/kernelgen-flagos> 安装 kernelgen-flagos 技能。"

   * **其他 AI 智能体客户端**："从 <https://github.com/flagos-ai/skills/tree/main/skills/kernelgen-flagos> 安装 kernelgen-flagos 技能。"

5. 验证技能是否安装成功："请验证 kernelgen-flagos 技能是否正常工作。"
